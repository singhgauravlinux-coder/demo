...
module.exports = {
  ...
  paths: {
    ...
    // ----------------------------------------------------------- language
    '/language': { get: { tags: ['language'], summary: 'List all supported UI languages', responses: { 200: ok('14 languages incl. en, es, fr, de, ar, zh, ja') } } },
    '/language/detect': {
      get: {
        tags: ['language'], summary: 'Resolve the best supported language for a client',
        parameters: [
          { name: 'lang', in: 'query', schema: { type: 'string', example: 'fr' }, description: 'Explicit override, skips Accept-Language parsing' },
          { name: 'acceptLanguage', in: 'query', schema: { type: 'string', example: 'fr-CA,fr;q=0.9,en;q=0.8' }, description: 'Overrides the Accept-Language header for testing' }
        ],
        responses: { 200: ok('Resolved language code plus metadata'), 400: err('Unknown language override') }
      }
    },
    '/language/strings': {
      get: {
        tags: ['language'], summary: 'UI string bundle for a locale',
        parameters: [{ name: 'lang', in: 'query', schema: { type: 'string', example: 'ja' } }],
        responses: { 200: ok('Key/value UI strings, falling back to English'), 400: err('Unknown language') }
      },
      post: {
        tags: ['language'], summary: 'UI string bundle for a locale (JSON body)',
        requestBody: jsonBody({ lang: str('ja') }, ['lang']),
        responses: { 200: ok('Key/value UI strings, falling back to English'), 400: err('Unknown language') }
      }
    }
  },
